...
public class WebInfClassesScannerTest {
	...
	public void testStartsSearchingAtWebInfClasses() {
        ...
        final InterceptorRegistry interceptorRegistry = mockery.mock(InterceptorRegistry.class);
        mockery.checking(new Expectations() {
			{
				one(context).getRealPath("");
				will(returnValue(myPath));
				one(scanner).scan(new File(new File(myPath), "WEB-INF/classes"));
				will(returnValue(new ArrayList<Resource>()));
				// TODO i forgot how to do it without the cast :)
				one(registry).register((List<br.com.caelum.vraptor.resource.Resource>) with(a(List.class)));
			}
		});
		new WebInfClassesScanner(context, scanner, registry, interceptorRegistry).loadAll();
